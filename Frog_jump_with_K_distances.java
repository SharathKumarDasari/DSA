//----dp memoization---
class Solution {
    public static int fun(int n,int dp[],int heights[],int k){
        if(n==0) return 0;
        if(dp[n]!=-1) return dp[n];
        int tjumps=Integer.MAX_VALUE;
        for(int i=1;i<=k;i++){
            if(n-i>=0){
                int jump=fun(n-i,dp,heights,k)+Math.abs(heights[n]-heights[n-i]);
                tjumps=Math.min(jump,tjumps);
            }
        }
        return dp[n]=tjumps;
    }
    ...
    public static void main(String args[]){
        ...
        int k=sc.nextInt();
        System.out.print(frogJump(arr,k));
